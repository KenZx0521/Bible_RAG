...
function p(texts, opts = {}) {
  ...
  return new Paragraph({
    ...
    spacing: { after: opts.after !== undefined ? opts.after : 50, before: opts.before || 0, line: opts.line || 224 },
    indent: opts.indent,
    children,
  });
}
...
function sectionHead(text) {
  return new Paragraph({
    ...
    spacing: { before: 100, after: 50, line: 228 },
...
function subHead(text) {
  return new Paragraph({
    spacing: { before: 60, after: 25, line: 228 },
...
const introContent = [
  ...
  // P3 - Contributions
  p(["This paper addresses this gap with three contributions:"], { after: 15 }),
  p([
    { text: "1) ", bold: true },
  ...
  ], { after: 15, indent: { left: 180 } }),
  p([
    { text: "2) ", bold: true },
    "Controlled empirical evidence that Gemma 3 4B [6], deployed locally at zero API cost, matches Claude Haiku 4.5 [7] within <1% on all 12 generation metrics when retrieval conditions are identical.",
  ], { after: 15, indent: { left: 180 } }),
  p([
    { text: "3) ", bold: true },
...
const resultsContent = [
  ...
  p([
    ...
    ". The maximum absolute delta is 0.0098 (answer point coverage). Of the 12 metrics, Gemma leads on 7 (BLEU, ROUGE-2, faithfulness, answer relevancy, context precision, context recall, answer correctness) while Claude leads on 5 (BERTScore, semantic similarity, ROUGE-1, ROUGE-L, answer point coverage). The bidirectional distribution of advantages confirms these are stochastic fluctuations rather than systematic model superiority. When high-quality context is provided through RAG, the LLM\u2019s role reduces to surface-level synthesis from evidence\u2014a free, locally deployed 4B-parameter model is functionally indistinguishable from a commercial API.",
  ], { after: 40 }),

  subHead("B. Retrieval Quality Determines Answer Quality"),
  p([
    "Table II reveals a clear monotonic relationship between retrieval and generation quality across all five question types.",
  ], { after: 20 }),

  // Table II caption
  new Paragraph({
    alignment: AlignmentType.CENTER,
    spacing: { before: 20, after: 30, line: 200 },
  ...
  // Per-type analysis
  p([
    "VERSE_LOOKUP achieves perfect retrieval (hit rate = 1.0, NDCG@5 = 1.0) via PostgreSQL exact match, yielding the highest APC (0.958/0.975). TOPIC questions also achieve perfect hit rate with high NDCG (0.983), producing strong APC (0.629/0.592). EVENT questions exhibit the lowest retrieval quality (hit rate = 0.750, NDCG@5 = 0.548)\u2014event narratives span multiple chapters, challenging fixed-window chunking. PERSON questions, despite perfect hit rate, show moderate NDCG (0.927) and low APC (0.303/0.287), suggesting Neo4j MENTIONS traversal retrieves broadly rather than precisely (highest F1@k = 0.778). This pattern across both models confirms that ",
    { text: "retrieval infrastructure, not model capability, is the performance bottleneck", italics: true },
    ".",
  ], { before: 20, after: 30 }),

  subHead("C. Metric Selection for RAG Evaluation"),
  p([
    "ROUGE-1/2/L scores are extremely low (<0.03), reflecting the paraphrased, generative nature of Traditional Chinese answers rather than failure. Semantic similarity (0.769) and BERTScore (0.653) better capture answer quality by measuring meaning preservation over lexical overlap. Among LLM-judged metrics, answer relevancy (0.73) and context recall (0.72) best differentiate question types. We recommend prioritizing semantic and LLM-judged metrics over n-gram metrics for cross-lingual or generative RAG systems.",
  ]),
];
...
function ref(num, text) {
  return p([
    ...
    { text, size: PT7 },
  ], { after: 10, line: 192, size: PT7, indent: { left: 280, hanging: 280 } });
}
