...
# Regex pieces
_CHAP_RANGE = re.compile(r"^(\d+)\s*[-–]\s*(\d+)\s*章?$")
...
def _parse_single_ref(book_name: str, spec: str) -> ParsedReference:
...
def _parse_book_segment(segment: str) -> list[ParsedReference]:
    ...
    for part in parts:
        ...
        if inner_book and inner_book != book_name:
            ...
            refs.append(_parse_single_ref(book_name, part))
    return refs
...
def parse_reference(reference: str) -> list[ParsedReference]:
    ...
    for segment in segments:
        ...
        if book_name is not None:
            ...
            ref = _parse_single_ref(last_book, segment)
            results.append(ref)
